from . import BotBehaviorModule, MentionTrigger, mention_target_self
...
yes_no_answers = [
	...
	"Mmm-hmm! Yeah.",
	"Th-there's just no way that could be!",
	"Hmm, well, do you think so?",
	"Probably!",
	"Oh no, um, I'm just not sure!",
	"Yeah! Definitely!",
	"No, I don't think so...",
	"Maybe! You never can tell",
	"That's just not possible, I'm really sorry!",
	"Well, yes, but only on Fridays, right? Heheh.",
	"Yeah... unless it's a Monday... then probably not..."
]

class ConversationModule(BotBehaviorModule):

	def __init__(self, bot_api, resource_root):
		help_text = "This module makes it so I can answer what you say! To use it, just mention my name at the start"
		help_text += " of a sentence."
		super().__init__(
			bot_api,
			name="converse",
			desc="Answers mentions",
			help_text=help_text,
			triggers=[
				MentionTrigger(mention_target_self())
			],
